...
<div class="container-fluid">
  <div class="col">
    <div class="">
       <div class="mb-5 col-lg-12 kwat_ern_bx dash-ref-user-link copyafilate_pge">
         <span class="matching_bonus_link">Copy Your Affiliate Link Here To Share 4NEW With Friends</span> <p><input type="text" class="ref-input" id="post-reflink" value="{{url('aff')}}?ref={{ Sentinel::getuser()->ref_token }}&page=home" readonly="">
      <span data-clipboard-target="#post-reflink" class="input-group-btn" onclick="copyreflink('#post-reflink')">COPY</span></p>
      </div>
      ...
      <div class="row">
            ...
            <div class="col-md-10">
              ...
              <select name="select-currency" id="payment-opt" class="form-control" style="font-family:'FontAwesome', arial;">
                ...
                <!-- <option class="dropdown-item" value="credit_card">
                  ...
                  <!--  <option class="dropdown-item" value="credit_card">
                  <i class="fab fa-bitcoin"></i> VISA</option> -->
                 <!--  <option class="dropdown-item" value="visa_card">
                  <i class="fab fa-bitcoin"></i> VisaCard</option> -->
                   <option class="dropdown-item" value="master_card">
                  <i class="fab fa-bitcoin"></i> MasterCard</option>
        ...
        <div class="col-md-10 master_card">
            <label>How many USD do you want to convert into Kwatts?</label>
            <div class="custom-icon input-group">
            <span> <i class="fas fa-dollar-sign"></i></span>
            <input type="text" class="form-control" id="usd_amount" name="usd_amount" value="" onkeyup="kwatt_balance(this.value,2)" placeholder="Enter number of USD" autocomplete="off">
...
<img src="https://www.clkmg.com/api/a/pixel/?uid=89815&att=2&ref=" height="1" width="1" />
 <input type="hidden" name="" id="last_transaction_time"
        value="{{isset($latest_data->created_at)?$latest_data->created_at : ''}}">
<form id="usd-form-user" action="{{ url('/credit-card-payment') }}" method="POST" 
name="paypalform">
           {{ csrf_field() }}
...
<form id="master-card-payment" action="{{ url('/master-card-payment') }}" method="get" 
name="paypalform">
         <!--   {{ csrf_field() }} -->
<!-- <input type="hidden" name="kwatt_amounts" id="master_kwatt_amounts" > -->
<input type="hidden" name="coin_amounts" id="master_coin_amounts" >
<input type="hidden" name="card" id="card_type" >
</form>
...
<script type="text/javascript">
    ...
    {
       var copyredreflink = document.getElementById("post-reflink");
       copyredreflink.select();
...
//$('.bitcoin').hide();
  $('select[name=select-currency]').change(function() {
    ...
    else if(currency=='master_card' || currency=='visa_card')
    {
        $('.master_card').show();
        $('.bitcoincash').hide();
        $('.litcoin').hide();
...
<div id="welcomemessage" class="modal modal-styled fade in modals-body">
          ...
          <div class="modal-content">
            ...
            <div class="logged_succcefuly">
             <span id="succfl_chck"><img src="{{ url('/') }}/assets/images/check.png" width="20" height="20"></span>
              <h4 class="text-center w-100">Welcome  {{Sentinel::getUser()->fullname}}</h4>
              ...
              <div class="modal-body theme-form">
                ...
                <div class="text-center">
                 <div class="support_dev">
                  <p>Thank you for joining 4NEW you now have 
                  @if(session('air_drop')==1)
                    $20*
                  @elseif(session('air_drop')==11)
                    $22*
                  @endif
                 worth of Tokens - Now is your opportunity to buy extra KWATT tokens and claim the 25% bonus</p>
                  <p style="font-size: 12px;">* To complete your ownership of your tokens you must fill out that KYC verification</p>
        ...
        @endif
        @if(session('paymentsuccess')==1)
        <script>
        
         $(document).ready(function(){
           //var message='<?php //echo session('success'); ?>';
          swal("Thanks for buying KWATT", "Your payment has successfully gone through", "success")
        });
      </script>
      @endif
@endsection
